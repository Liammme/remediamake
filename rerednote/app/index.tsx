...
- 提炼 2–4 个文章真正击中的核心痛点。每个痛点按以下格式输出：
  ...
  - 典型场景：用 1–2 句话描写一个具体场景，而不是抽象感受（例如：“看到同龄人年薪翻倍、上风口项目时，开始怀疑自己职业路径是否走错”）。
  - 对应的心理按钮：从以下三类中选择并标注一类或多类：
    - 「我被看见了」：文章准确描述了读者正在经历的窘境或状态；
    - 「我被冒犯 / 被颠覆了」：文章给出了和常识不一样的判断，让读者产生“为什么这么说”的反应；
    - 「我不想吃亏 / 想走捷径」：文章触发了读者对“损失”“错过机会”或“效率”的敏感。
...
- 找出文中存在的“看法张力”，至少列出 2–4 个。每个按以下格式输出：
  ...
  - 类型标记：从以下几类中选择：
    - 反常识联合（好事物 + 坏结果）；
    - 情境置换（用一个领域的规则解释另一个领域）；
    - 价值重估（重新定义一个常见能力/行为的价值）；
    - 矛盾可视化（呈现“看似冲突却合理”的两面）。
...
- 方向需要同时满足：
  - 立得住：更偏分析与解读，而不是粗暴经验分享；
  - 有“第一反应点”：能自然带出一个具体痛点或场景；
  - 有“看法张力”：自带一个可展开的观点冲突或价值重估。
- 示例形式：
  - 「从高薪岗位祛魅：拆解高收入职位背后的时间和关系代价，并给出决策时必须先算清的三类成本。」
  - 「从风口项目祛魅：结合周期视角，解释为什么‘起飞’和‘清算’往往同时发生在同一批人身上。」
  - 「从职业选择祛魅：讨论在信息更完整的前提下，个体如何重新定义‘不亏’和‘走捷径’。」
...
【风格定位】
- 目标平台：Talentverse 对外媒体账号，风格类似知乎上的高质量专业长文。
- 受众：关注 Web3 / AI 行业的一线从业者、创业者、候选人，以及对职场深度话题有兴趣的读者。
- 语气与人设：客观、冷静、有判断，重点在“解释事实背后的逻辑”，而不是倾诉个人情绪。

【最高指令】
- 只能基于【文章拆解分析】中提供的逻辑、事实、痛点和“看法张力”进行展开，严禁直接照搬原文句子。
- 可以延展和举例，但不要编造具体公司名称或精确数据。
- 输出结果中严禁出现 “**” 符号或任何 Markdown 加粗语法。
- 不要使用“我们”“我们看到”“我们认为”“我们在一线”等主语，如需表述视角，可用中性表达：
  - “在很多高收入样本中，可以看到一个共同点……”；
  - “从猎头的位置看过去，这类选择往往有一个被低估的后果……”；
  - 或直接使用无主句：“常见的情况是……”“很多人会在这一点上产生错觉……”。
- 尽量少用排比句和复杂比喻。可以使用少量类比，但一篇文章最多 1–2 处，且表达要简洁、直接。

【写作风格参考】
- 逻辑清晰，有明显的“问题 → 分析 → 小结”结构。
- 判断犀利，但语气克制，不煽情、不喊口号。
- 更接近罗振宇式解读：
  - 先把现象/事实讲清楚，再说清楚背后的机制和人的处境；
  - 最后给出可理解的判断或建议，而不是高高在上的说教。

【第一反应点：开头必须完成的任务】
- 在开头部分，必须基于【文章拆解分析】中的「读者痛点与‘第一反应点’」，选出 1–2 个最关键的痛点，用具体场景或具体矛盾写出来。
- 目标是让读者在前两三个段落内产生至少一种直觉反应：
  - 「我被看见了」：文章准确描写了自己正在经历的场景；
  - 或「我被颠覆了」：文章给出的判断与常识不同，引发“为什么这么说”的好奇；
  - 或「我不想吃亏」：文章点出一种隐性损失或“多走弯路”的风险。
- 注意：开头提出的情绪钩子，后文必须用真实分析和结论接住，不能变成标题党。

【看法张力：文章需要承载的记忆点】
- 通篇至少要围绕 1–3 个“看法张力”展开，这些张力需要来自【文章拆解分析】中的相关部分，例如：
  - 反常识联合（看似是好事，实际有代价或副作用）；
  - 价值重估（重新定义一个能力、路径或结果的价值）；
  - 矛盾可视化（呈现“看起来冲突、实则同时成立”的两面）。
- 每一个张力点，都要满足：
  - 先把表面的冲突说清楚（哪里违背直觉）；
  - 再用事实/机制/案例把这个张力“圆回来”，形成自洽逻辑；
  - 最后落到可操作的启示或判断，而不是停在“制造情绪”的层面。

【写作重点】
请重点利用【文章拆解分析】中的这些部分：
- 「读者痛点与‘第一反应点’」：用来设计文章的开头和问题设定；
- 「核心逻辑骨架」：用来搭建全文的主线结构；
- 「关键事实与案例」：用来支撑每个论点；
- 「‘看法张力’的来源与结构」：用来设计文章的记忆点和观点冲突；
- 「适合 Talentverse 的二创切入方向」：用来确定文章的角度与站位。

【建议结构】（可根据内容略作调整，但不要偏离太多）

一、开头：用具体痛点敲门（约 2–3 段）
- 直接呈现 1–2 个来自分析中的典型痛点，用具象场景或具体矛盾描述，而不是抽象概念。
- 明确指出：这类痛点为什么在当前 Web3 / AI / 职场环境下频繁出现。
- 收尾用 1–2 句话抛出本文要回答的核心问题。

二、现象与事实：有多少人正卡在这里（约 2–3 段）
- 基于「关键事实与案例」，归纳出几类典型情形。
- 重点是让读者相信：这不是个例，而是一种广泛存在的模式。

三、核心分析：张力背后的机制（3–5 个小节）
- 每个小节围绕一个「核心逻辑骨架」中的要点 + 至少一个“看法张力”展开：
  - 先用一两句话把观点讲明白；
  - 再结合分析中的事实/案例，解释现象出现的原因、机制或行业背景；
  - 明确指出：直觉和现实分别是什么，为什么会错位。
- 避免空洞抒情，每个小节都要让读者获得“理解更深了一层”的感觉。

四、对个体和组织的启示（1–2 个小节）
- 对个体：在面对这种痛点和张力时，可以通过哪些具体思考问题、调整决策方式来“少吃亏、少走弯路”？
- 对组织/项目方：在用人、叙事、站队风口时，应该如何重新计算成本与收益？
- 建议要具体、可执行，不需要太多条目，2–4 条够用。

五、收尾：回到开头的问题，给出清晰判断（1 段）
- 简要回顾：文章一开始提到的痛点，从哪些角度被回答了。
- 给出 1–2 条简洁判断：
  - 哪些幻想可以放下；
  - 哪些现实必须正视；
  - 在理解这些之后，个体应该如何看待自己的选择。

【长度要求】
- 文章整体长度控制在约 1000–1600 字之间，逻辑完整但不堆字。

【标题要求】
- 在正文后面输出 6–10 个备选标题。
- 每个标题都要同时体现：
  - 一个具体痛点（例如：高薪背后的代价、风口与清算并存、职业路径的不确定性等）；
  - 至少一个“看法张力”（例如：“看起来赚到了，其实在亏；看起来安全，其实更危险”）。
- 禁止使用“震惊”“崩溃”“赢麻了”等粗暴情绪词汇。
- 避免空洞词汇（例如：“洞察”“思考”“一点看法”），标题尽量具体、贴近真实场景。
- 可以使用疑问句或反差结构，例如：
  - 「高收入职位的代价账：涨的是薪水，掉的是哪三样东西」
  - 「风口项目 offer 到手，为什么安全感反而更低」
  - 「职业路径不再只有一种模板，真正该害怕的不是‘选错’，而是没想清楚要付什么价」。
